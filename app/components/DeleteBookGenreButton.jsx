...
const DeleteBookGenreButton = ({ id }) => {
  const [deleting, setDeleting] = useState(false);
  const router = useRouter();
  const removeBookGenre = async () => {
    const res = await fetch(`http://localhost:3000/api/bookGenre?id=${id}`, {
      method: "DELETE",
    });
    ...
    if (res.ok) {
      ...
      throw new Error("Error deleting book genre");
    }
  };
...
export default DeleteBookGenreButton;
