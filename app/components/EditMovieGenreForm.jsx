...
const EditMovieGenreForm = ({id, movieGenreName, movieGenreDescription}) => {
  const [name, setName] = useState(movieGenreName);
  const [description, setDescription] = useState(movieGenreDescription);
  ...
  const handleSubmit = async (e) => {
    ...
    if (name == "" || description == "") {
      ...
      setWrong(false);
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/movieGenres/${id}`, {
          method: "PUT",
          headers: {
...
export default EditMovieGenreForm;
